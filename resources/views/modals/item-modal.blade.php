            ...
            <div class="p-6">
                ...
                <form hx-post="{{ route('customer.cart.add') }}" 
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <!-- Add to Cart Button -->
                        <button type="submit" 
                                @if(!$item->available || $item->is_86ed) disabled @endif
                                class="@if(!$item->available || $item->is_86ed) bg-gray-400 cursor-not-allowed @else bg-primary-600 hover:bg-primary-700 @endif text-white font-semibold py-3 px-6 rounded-lg transition-colors">
                            @if(!$item->available || $item->is_86ed)
                                Unavailable
                            @else
                                Add to Cart
                            @endif
...
function updateCartCount() {
    // Update cart count in header - fetch current cart count from server
